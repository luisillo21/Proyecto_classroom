<?php
	function valida_permisos($conexion, $accion)
	{
		...
		else
		{
			...
			{
				/*
				if(($row['per_agregar']=="S")   AND ($accion=="A")) {return 1;}
				if(($row['per_consultar']=="S") AND ($accion=="C")) {return 1;}
				if(($row['per_modificar']=="S") AND ($accion=="M")) {return 1;}
				if(($row['per_eliminar']=="S")  AND ($accion=="E")) {return 1;}
				if(($row['per_imprimir']=="S")  AND ($accion=="I")) {return 1;}
				echo "imprimir ".$row['per_imprimir'];
				echo "accion ".$accion;
